...
use a new wrapper (`Sys.with_async_exns`) that turns the interrupting exception
back into an ordinary one. We call these "interrupting exceptions" below.

## Bottom line

- **Frama-C turns Ctrl+C into an exception in two different ways, and only one of
  them is interrupting.**
  - At start-up the batch driver runs
    ...
    interrupting exception, caught all over Frama-C with ordinary `try … with`.
...
- **Throwing side — almost nothing to do, with one exception.** Where Ctrl+C goes
  ...
  `Sys.Break` an interrupting exception, so no code change is needed to throw it.
  **The one place that needs a throwing-side check is Eva's own `SIGINT` handler**
  ...
  already the kind OxCaml turns into an interrupting one, so it is fine, but it is
...
- **Two spots that are safe today only by luck (and therefore fragile):** the
  ...
  interrupting `Sys.Break` would skip; they are not day-one bugs only because the
...
### `Sys.Break` is interrupting in Frama-C (`Sys.catch_break true`)
...
That is exactly the interrupting-exception pattern: today the runtime lets the
...
interrupting path and **none** of those handlers catch it — it goes to the nearest
...
interrupting in `-server` mode too.

Two consequences:
1. **Throwing side — already handled** for the standard `catch_break` route.
   OxCaml already makes the `Sys.Break` escaping that handler an interrupting
...
2. **Catching side — the work.** Every `with Sys.Break` / catch-all handler stops
   firing; a `Sys.with_async_exns` wrapper is needed to turn the interrupting
...
- **`SIGINT` → `interrupt _ = warn (); raise Sys.Break`** — raises `Sys.Break`
  *directly from inside the signal handler*. This is the interrupting case. Because
  it is `Sys.Break` specifically, OxCaml treats it the same way it treats the one
  installed by `Sys.catch_break` — it becomes an interrupting exception for free,
...
- **`SIGUSR1` → `stop _ = warn (); kill ()`** where `kill ()` just sets
  ...
  the main stack — so it is *not* an interrupting exception and is unaffected by
...
`Fun.protect ~finally` that an interrupting `Sys.Break` would bypass. After tracing
...
- **`at_exit` runs when `Sys.Break` (and `Stack_overflow`) is uncaught** — verified
  ...
  *and* from an uncaught `Stack_overflow` both ran the `at_exit` callback (exit
  ...
  still runs on an interrupting-uncaught termination *if OxCaml's uncaught path
...
*interrupting* exception frees the resource but keeps the Break flying past
...
- **`Sys.Break` really is interrupting in Frama-C** — confirmed via
...
- **Eva's `SIGINT` handler raises `Sys.Break` directly; its `SIGUSR1` handler does
  ...
  interrupting — so no `Sys.raise_async` is needed.
...
- **`at_exit` runs on uncaught `Sys.Break`/`Stack_overflow`** — verified by
  ...
  survive an interrupting-uncaught termination, *if OxCaml's uncaught path matches*.
...
- **Where to put the wrapper — the real design call, and it differs by mode.**
  ...
  - *Server:* a `Sys.with_async_exns` **per request** (around `proc.handler` in
    ...
    Without this, an interrupting `Sys.Break` bypasses the per-request handler and
...
- **OxCaml confirmations** (need the runtime, not the source):
  - that a *hand-installed* `Signal_handle` raising `Sys.Break` (Eva's `interrupt`
    handler) gets the same free interrupting treatment as `Sys.catch_break` — if
    not, Eva's `SIGINT` handler needs `Sys.raise_async`;
  - that `at_exit` runs on interrupting-uncaught termination (so the
    prover/temp-file clean-up and json/log flush survive).

## Checked and fine

- **`Stack_overflow`** — listed as interrupting by the doc; Frama-C is deeply
  recursive (CIL traversals, the Eva dataflow), but there are **zero** catch sites.
  It already crashes uncaught; interrupting or not, no recovery-path change. On its
...
In priority order. There is essentially one interrupting exception (`Sys.Break`)
...
2. **Make Eva's `Signal.protect` interruption-aware.**
   ([`signal.ml:58-76`](https://git.frama-c.com/pub/frama-c/-/blob/44585036cb05125b3128a173c7d98881c4bbdb42/src/plugins/eva/src/engine/signal.ml#L58-L76))
   Rebuild it so its clean-up (save partial results) runs on an interrupting
...
5. **Confirm on the OxCaml runtime** (not derivable from source): that a
   ...
   as interrupting like `Sys.catch_break` — otherwise add `Sys.raise_async` there;
   and that `at_exit` runs on interrupting-uncaught termination (so the
   prover/temp-file clean-up and json/log flush survive).

**Sequencing note:** the throwing side needs essentially no work (`Sys.catch_break`
and the hand-installed `Sys.Break` are already interrupting on OxCaml), so (1) the
...
**Do *not*** rebuild the helpers so they turn the interrupting exception into an
