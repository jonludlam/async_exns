...
- Two asynchronous exceptions can travel the new route, and **only in some
  modes**:
  - **`Control.Timeout`**, raised **from inside a `SIGALRM` signal handler** by
    ...
    Unix. This is a genuine asynchronous raise.
  - **`Sys.Break`** (Ctrl+C). How it is produced depends on the mode, and this
    ...
    - The general `Control.check_for_interrupt` checkpoint used throughout the
      ...
      code — that is *not* an asynchronous exception and is unaffected.
...
- **Throwing side:** the one place that needs a throwing-side change is the
  ...
  `Sys.Break` asynchronous automatically.
- **Good news on the catching side:** Rocq is unusually well-prepared. It does
  ...
  **already** built on a masking bracket (`Memprof_coq.Masking.with_resource`),
  ...
  handled.
- **No genuine state-corruption bug found.** The places that catch asynchronous
...
`Sys.Break`. Under OxCaml this `Sys.Break` is automatically made asynchronous,
...
**not asynchronous**, and therefore unaffected by the change. This is the
...
**from inside the `SIGALRM` handler** — so it is genuinely asynchronous. It is
...
2. **Catching side:** the `try … with Timeout` *immediately around the body* in
   `unix_timeout` will no longer fire for an asynchronous `Control.Timeout`. So
   this same function needs the `Sys.with_async_exns` wrapper placed around
   `f x` (turning the asynchronous `Timeout` back into an ordinary one that the
...
output). It catches no asynchronous exception itself and is unaffected.

### The recovery loops — where asynchronous exceptions are caught today

Each long-lived mode catches everything (including today's asynchronous
...
- **Editor/IDE backend (`rocqide` document manager).** Its own `SIGINT`
  ...
  When it is on, that `raise Sys.Break` *is* an asynchronous raise; when it is
  off, the handler only sets the cooperative flag. The asynchronous `Sys.Break`
  ...
  Under the new rules the asynchronous `Sys.Break` bypasses that `with any`, and
  ...
  — also depends on the throwing side: it gets the asynchronous `Sys.Break` for
...
- **One-shot compile (`coqc` / `rocq compile`).** `coqc_run` wraps the whole
  ...
  terminate as an uncaught asynchronous exception. `at_exit flush_all` still
...
So an asynchronous exception that skips an intermediate handler does not leave a
dangling "half-applied tactic" — the next thing the recovery loop does is reset
to a saved snapshot.

### The `noncritical` discipline — already excludes asynchronous exceptions
...
sites; today they deliberately re-raise an asynchronous exception. Under the new
rules they would not even be entered by an asynchronous exception — the outcome
is the same (the asynchronous exception keeps propagating), so these are not a
...
Today this *already* refuses to treat an asynchronous exception (Ctrl+C, a
timeout, stack overflow, out-of-memory) as "the command failed as expected": the
`is_async` guard re-raises it. Under the new rules an asynchronous `Sys.Break`
...
cleanup would no longer run when an asynchronous exception passes through. After
...
1. **Which "Ctrl+C exception" you have depends on the mode and the moment.** The
   ...
   stock `Sys.catch_break` runtime handler (asynchronous), by the IDE backend's
   own handler when a request is being evaluated (asynchronous), and by
   `check_for_interrupt` reading a flag (cooperative, *not* asynchronous). Only
...
2. **Today's catching is nondeterministic.** When `Sys.Break` or `Control.Timeout`
   is asynchronous, it fires at an arbitrary safe point, so whichever matching
...
3. **The handlers form a print-then-recover chain, with state restored
   ...
   *recovery decision* (keep going) are in different frames, and neither relies
   on a `finally` inside the failing command. So an asynchronous exception that
...
4. **The change splits in two.** Independently of where the wrapper goes, the
   inner annotate/print frames are skipped by an asynchronous exception — but
...
the cleanup, then re-throws the interruption still-asynchronous. Rocq already has
...
catches the (now asynchronous) `Control.Timeout`. After that, one wrapper per
...
matter whether the bracket re-throws the interruption as still-asynchronous or
as ordinary — the resource is released either way. But do **not** make the
cleanup bracket convert asynchronous→ordinary as a side effect: that would let a
downstream `with _ ->` swallow a Ctrl+C again, which is the very thing the new
model is meant to stop. Use a masking bracket that re-throws still-asynchronous,
...
- **No proof/environment state is repaired inside command-level handlers.** The
  ...
  command. So an asynchronous exception skipping the inner frames does not leave
...
- **The `Fail` vernacular already refuses to absorb asynchronous exceptions**
...
- **`at_exit` runs on an uncaught asynchronous exception** — verified empirically
  ...
  OxCaml's asynchronous-termination path matches stock uncaught behaviour* (worth
...
- **`Stack_overflow`** — the kernel is deeply recursive and can raise it, but
  ...
  the top level. No recovery path regresses. It does, like any asynchronous
...
- **The ~170 `with e when noncritical e ->` handlers** — `noncritical` already
  excludes every asynchronous exception, so they deliberately do not catch them
  today; under the new rules they are not entered by an asynchronous exception
...
In priority order. The set of asynchronous exceptions to reason about is small:
...
2. **Add a per-command / per-request `Sys.with_async_exns` wrapper to each
   ...
   asynchronous, which OxCaml provides automatically.
...
5. **Confirm on the OxCaml runtime** (not derivable from source): that `at_exit`
   runs on asynchronous-uncaught termination, and that the `Sys.catch_break`
...
brackets in (4) into asynchronous→ordinary converters — that would re-enable
