...
(`Sys.with_async_exns`, which turns an interrupting exception back into an ordinary
one so normal handlers see it again) the exception flies straight past every normal
handler and either reaches that wrapper or stops the program. Below we call them
...
- **The genuine bug: Ctrl+C during evaluation kills the whole utop session instead of
  ...
  continues — you get your prompt back. Under OxCaml the interrupting `Sys.Break`
...
- **Throwing side for Ctrl+C: nothing to do.** utop installs its SIGINT handler with
  ...
  escaping that handler to be an interrupting exception. So Ctrl+C propagates the new
...
- **Throwing side for SIGHUP/SIGTERM: a real change is needed.** utop installs its own
  ...
  give it the interrupting treatment automatically: under the new model an ordinary
...
- **Only two interrupting exceptions are actually in play:** `Sys.Break` (Ctrl+C
...
### `Sys.Break` is interrupting during evaluation (`Sys.catch_break true`)
...
exactly the interrupting-exception pattern. The comment right above it —
...
1. **Throwing side — already handled.** Because utop uses the standard
   ...
   an interrupting exception. utop needs no code change to throw it the new way.
2. **Catching side — the bug.** The interrupting `Sys.Break` does **not** match the
   ...
   wrapper is needed to turn the interrupting exception back into an ordinary one so
...
get the interrupting treatment automatically and would terminate the program rather
...
`Stack_overflow` becomes interrupting and bypasses that arm exactly as `Sys.Break`
...
2. **There are two different `Sys.Break`s that look identical in the source.** The one
   ...
   `Sys.catch_break` SIGINT handler) becomes interrupting and stops being caught. A
...
3. **The handlers form a chain, and the interrupting exception skips all but the
   outermost.** A Ctrl+C deep inside a running expression unwinds, innermost-out:
   ```
   Sys.with_async_exns wrapper                                   <- to be added (turn interrupting -> ordinary)
...
Use `Sys.with_async_exns` to convert the interrupting exception back into an ordinary
...
|-----------|-------|-----|------------|
| Catches the exception to decide what to do next — *consumes* the Ctrl+C/`Stack_overflow` to report it and keep looping | the console eval catch-all ([`uTop_main.ml:770-802`](https://github.com/ocaml-community/utop/blob/8cd6716d0f90efd67da90afa5bc31c7abe9a7a57/src/lib/uTop_main.ml#L770-L802)) and the Emacs one ([`:986-1002`](https://github.com/ocaml-community/utop/blob/8cd6716d0f90efd67da90afa5bc31c7abe9a7a57/src/lib/uTop_main.ml#L986-L1002)) | wrap the **evaluation step** in `Sys.with_async_exns` so an interrupting `Sys.Break`/`Stack_overflow` turns back into an ordinary exception that the existing `with exn ->` catches — the loop then reports it and continues exactly as today | small, but placement is the design call |
| Resource / state restore — only needs "the finaliser must run" | the two formatter save/restore wrappers ([`uTop.ml:155-184`](https://github.com/ocaml-community/utop/blob/8cd6716d0f90efd67da90afa5bc31c7abe9a7a57/src/lib/uTop.ml#L155-L184), [`:186-214`](https://github.com/ocaml-community/utop/blob/8cd6716d0f90efd67da90afa5bc31c7abe9a7a57/src/lib/uTop.ml#L186-L214)), the env/snapshot restore in `check_phrase` ([`:375-390`](https://github.com/ocaml-community/utop/blob/8cd6716d0f90efd67da90afa5bc31c7abe9a7a57/src/lib/uTop.ml#L375-L390)), `use_output`'s temp-file removal ([`:796-809`](https://github.com/ocaml-community/utop/blob/8cd6716d0f90efd67da90afa5bc31c7abe9a7a57/src/lib/uTop.ml#L796-L809)), the stash `close_out` ([`:683-687`](https://github.com/ocaml-community/utop/blob/8cd6716d0f90efd67da90afa5bc31c7abe9a7a57/src/lib/uTop.ml#L683-L687)), the Emacs SIGINT restore ([`uTop_main.ml:934-942`](https://github.com/ocaml-community/utop/blob/8cd6716d0f90efd67da90afa5bc31c7abe9a7a57/src/lib/uTop_main.ml#L934-L942)) | `new_protect` so the restore/removal runs even when an interrupting exception unwinds, then re-throw the interruption unchanged | mechanical |
...
and then re-throws the *interrupting* exception frees the resource but keeps the
...
re-enable utop's many `with exn ->`/`with _ ->` arms to swallow a future interrupting
...
- **`Sys.Break` really is interrupting in utop during evaluation** — confirmed via
...
- **Throwing side for Ctrl+C needs no change** — OxCaml already makes the `Sys.Break`
  raised by the standard `Sys.catch_break` an interrupting exception. (This makes the
...
- **History save is backstopped by `at_exit`** — `init_history` registers the save via
  ...
  the bug above, history is still saved — *if* OxCaml's uncaught-interrupting path runs
...
- **Where to put the `Sys.with_async_exns` wrapper (the real design call).** The
  ...
  and in the Emacs
  [`process_checked_phrase`](https://github.com/ocaml-community/utop/blob/8cd6716d0f90efd67da90afa5bc31c7abe9a7a57/src/lib/uTop_main.ml#L986-L1002)
  — so that an interrupting `Sys.Break`/`Stack_overflow` becomes an ordinary exception
...
- **OxCaml confirmations** (need the runtime, not the source): (1) does `at_exit` run on
  interrupting-uncaught termination (so the history save survives if the session dies)?
  (2) does the terminal mode get restored when an interrupting exception unwinds out of
...
- **`Stack_overflow`** — listed as interrupting by the doc; realistic in utop (user
...
- **The pure `with _ -> None` guards** in the rewrite-rule machinery
  ...
  release no resource and restore no state; an interrupting exception simply skips the
...
- **The Emacs output-copy `Thread.create`**
  ...
  handler and raises nothing interrupting. Not in scope.
...
- **`expunge/modules.ml` `Fun.protect`** — a build-time helper that closes a file it
  read; not part of the shipped interactive loop and not reached by an interrupting
...
interrupting `Sys.Break` on OxCaml), so the headline work is one wrapper plus the
...
1. **Restore Ctrl+C recovery — the one genuine bug.** Wrap the **evaluation step** in
   ...
   interrupting `Sys.Break`/`Stack_overflow` becomes ordinary, the catch-all reports it,
...
4. **Confirm on the OxCaml runtime** (not derivable from source): (a) `at_exit` runs on
   interrupting-uncaught termination, so the history save survives a killed session;
   (b) the terminal mode is restored when an interrupting exception unwinds out of
...
rebuild the clean-up helpers to turn the interrupting exception into an ordinary one:
