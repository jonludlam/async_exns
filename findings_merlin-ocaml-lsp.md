...
An asynchronous exception is an interrupting event — running out of stack,
pressing Ctrl+C, a timer firing, the garbage collector calling a finaliser —
that can surface at almost any point in the program. Today these interrupting
...
is a new wrapper, `Sys.with_async_exns`, that turns one of these interrupting
...
*deliberately* throws an interrupting exception from a signal handler or
finaliser. Below these are called "interrupting exceptions".

## Bottom line

- **The only interrupting exception that can actually reach either program is
  ...
  interrupting exception** in scope — just `Stack_overflow`, which the runtime
...
- **Both already keep signals from becoming exceptions.** merlin ignores SIGPIPE
  ...
  the only interrupting exception that can reach either tool is `Stack_overflow`.
- **Genuine bug — merlin's long-lived `server` mode corrupts itself after one
  ...
  interrupting `Stack_overflow` skips the `~finally`: the "in use" flag stays
...
- **Recovery is lost — both tools' per-request catch-all stops catching, so the
  ...
  the backend keeps serving. Under the new model the interrupting
...
- **Safe today only by luck (fragile):** a handful of small clean-up/restore
  ...
  An interrupting `Stack_overflow` skips each of these; in a long-lived process
...
- **Cooperative cancellation is not affected.** ocaml-lsp's `$/cancelRequest`
  ...
  Neither is an interrupting exception; both are unchanged by the new model.
...
That leaves **`Stack_overflow`** as the single interrupting exception either tool
...
so if an interrupting `Stack_overflow` skips the `~finally`, the flag is never
...
- ocaml-lsp: each request handler runs inside `Fiber.map_reduce_errors
  ...
  interrupting `Stack_overflow` bypasses.
...
([`workspace_symbol.ml:212-256`](https://github.com/ocaml/ocaml-lsp/blob/b90cbe7cdfb096eaf32df92b8db6681c428a5643/ocaml-lsp-server/src/workspace_symbol.ml#L212-L256)).
Both are ordinary, deliberate, synchronous mechanisms — they are not interrupting
...
the fiber error machinery), via an arm that an interrupting `Stack_overflow`
...
2. **The handlers form a clean-up-then-recover chain.** A `Stack_overflow` deep
   ...
   Under the new model the interrupting `Stack_overflow` skips **all** of these
...
- **Dependent on where a wrapper goes:** whether the request loop recovers at
  all. With no `Sys.with_async_exns` in the loop, an interrupting
...
finaliser and re-throws the *interrupting* exception unchanged frees the resource
but keeps the exception travelling the interrupting path (so you still need the
...
code has many — to swallow a future interrupting exception, against the model.
Best approach: interruption-safe helpers that re-throw the interrupting exception
...
- **The only interrupting exception in scope is `Stack_overflow`** — no
  `Sys.catch_break`, no `setitimer`/`Unix.alarm` timeout exception, no custom
  interrupting exception in either repo's own code.
...
- **Cooperative cancellation is unaffected** — `$/cancelRequest` fires a cancel
  ...
  exception; neither is interrupting.
...
- **Wildcard `with _ ->` handlers in merlin's analysis code** (completion,
  ...
  a single analysis step and fall back; under the new model an interrupting
...
1. **Add a per-request `Sys.with_async_exns` boundary in each request loop**
   ...
   ocaml-lsp
   [`jsonrpc_fiber.ml:184-270`](https://github.com/ocaml/ocaml-lsp/blob/b90cbe7cdfb096eaf32df92b8db6681c428a5643/jsonrpc-fiber/src/jsonrpc_fiber.ml#L184-L270)),
   so an interrupting `Stack_overflow` in one request is turned back into an
...
2. **Fix the merlin per-query state bracket (the genuine bug).** Make
   ...
   interrupting `Stack_overflow` unwinds — i.e. an interruption-safe `new_protect`
...
3. **Make the remaining clean-up helpers interruption-safe (hygiene).** merlin's
   ...
   not the call sites). Have them re-throw the interrupting exception unchanged,
   **not** convert it to an ordinary one (which would re-enable merlin's many
   downstream `with _ ->` arms to swallow a future interrupting exception).
...
clean-up helpers to convert the interrupting exception into an ordinary one —
merlin's analysis code is full of downstream catch-all handlers that would then
silently swallow a future interrupting exception, defeating the model.
