...
which turns an interrupting exception back into an ordinary one so normal handlers
see it again). We call these "interrupting exceptions" below.
...
- The **only** interrupting exception Alcotest can actually meet is `Stack_overflow`
...
- **Alcotest is a per-test recovery loop, and that is what the change affects.** Each
  ...
  test. Under OxCaml an interrupting `Stack_overflow` no longer matches that wildcard:
  instead of one failed test, **the whole run aborts**. This is a real behaviour change.
- **The clean-up that an interrupting `Stack_overflow` would skip is the restoration
  ...
  run only on the normal path; an interrupting `Stack_overflow` skips both, leaving the
...
- **One place catches the exception to decide what to do next**: the top-level
  ...
  `Cmd.eval_value ~catch:and_exit`). Under OxCaml an interrupting `Stack_overflow`
...
for log capture. None of that raises an exception from an interrupting context.
...
So the only interrupting exception left is the runtime's own `Stack_overflow`, which
...
An interrupting `Stack_overflow` raised inside the test body bypasses the `with e ->`
...
about to die from the uncaught interrupting exception, so the in-memory leak is moot —
...
- **Dependent on where a wrapper goes:** if Alcotest wants to *preserve* the current
  ...
  interrupting exception back into an ordinary one at a chosen point so that
...
|-----------|-------|-----|------------|
| Code that catches the exception to decide what to do next — *consumes* a `Stack_overflow` to turn it into a recorded per-test failure and continue | `protect_test` per-test catch ([`core.ml:183-195`](https://github.com/mirage/alcotest/blob/b26e58f62c93895f24a5ee48b550f274db74ee55/src/alcotest-engine/core.ml#L183-L195)); the top-level Cmdliner `~catch` ([`cli.ml:161-171`](https://github.com/mirage/alcotest/blob/b26e58f62c93895f24a5ee48b550f274db74ee55/src/alcotest-engine/cli.ml#L161-L171)) | wrap the per-test run in a `Sys.with_async_exns` boundary so an interrupting `Stack_overflow` is converted back to an ordinary one *before* it reaches `protect_test`'s wildcard, preserving "one failed test, keep going" | design call (where to put the boundary, and whether to bother for `Stack_overflow`) |
...
— it runs the clean-up even when an interrupting exception unwinds, then re-throws the
...
the test body but sits *below* `protect_test`'s wildcard — i.e. convert the interrupting
...
on the interrupting path (you still need the explicit `Sys.with_async_exns` per test if
...
`with e ->` to swallow a future interrupting exception, against the new model. So:
...
- **`at_exit` runs on an uncaught `Stack_overflow`** — verified empirically on stock
  ...
  still fires on an uncaught interrupting termination — *provided OxCaml's
...
- **Whether to preserve the "a stack-overflowing test is just a failed test" behaviour,
  ...
  around the test body, below `protect_test`'s wildcard, so an interrupting
...
- **OxCaml confirmations** (need the runtime, not the source): (1) that `at_exit` runs
  on interrupting-uncaught termination (so the stderr flush survives); and (2) the
...
- **`Stack_overflow`** — the one realistic interrupting exception. Covered above: today
...
- **Other wildcard / `exception _` handlers** — the remaining ones are not state
  ...
  overflows the stack while inside `check_raises`, under OxCaml that interrupting
...
- **JavaScript build** — `alcotest`'s
  ...
  a JavaScript runtime, so the interrupting-exception concerns are a native-runtime
  matter; the JS build is essentially out of scope.

## Recommendations

In priority order. The whole picture is small: there is exactly one interrupting
...
1. **Decide whether a per-test stack overflow should remain a single failed test rather
   ...
   so an interrupting `Stack_overflow` is converted back to an ordinary one and the
...
2. **Make the stream restoration interruption-safe.** Rewrite
   ...
   run even when an interrupting exception unwinds (an interruption-safe bracket whose
...
4. **Confirm on the OxCaml runtime** (not derivable from source): that `at_exit` runs on
   interrupting-uncaught termination (so the final stderr flush survives), and — together
...
turns the interrupting exception into an ordinary one — that would re-enable
`protect_test`'s wildcard (and every other `with e ->`) to swallow a future interrupting
